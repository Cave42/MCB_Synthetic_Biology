...
The introduction of protein language models (PLMs) has been made huge impacts in the field biology.
...
A masked language diffusion model will be built trained on the capsid proteins of rhinovirus, the dataset will include all the serotypes of rhinovirus
The model will be also trained on metagenomic sequencing data for rhinovirus from SRA, to encompass additional mutations. 
To further expand this dataset, other Picornaviridae capsid proteins will be included.

This protein language model will then be fine-tuned on the phage display results to predict and generate proteins that illicit a high immune response. 
The model's sequence embeddings will contain proteins of similar function clustering together and will be labeled by rhinovirus serotype. 
...
ESM3 will be also fine-tuned on PhiP-Seq expression data, ESM3 will be used instead of ESM2 as it can be trained on 3D structural information.
This much larger model will be tested against the previously generated model to test if a model trained on hundreds of millions of proteins will outperform a smaller model trained on a smaller more specific dataset.

=== Aim 3 Predict epitope sites and produce new phage libraries

Selected novel proteins generated by the model will then be used to generate a new library with alanine scanning preformed as well to test which residues are essential to illicit immune response. 
These new proteins will be tested against the sera samples from the diverse cohort of adults to see which newly generated what generate the biggest immune response across patients.
Proteins that illicit an immune response to the most patients will be used to further fine tune the model.
This will further optimize the model to generate proteins that not only illicit a strong immune response, but are also recognized by a majority of the sera, from adults who have been infected with different serotypes of rhinovirus.

This feed forward process will help identify likely proteins for a potential vaccine candidates, where more in depth testing can be preformed on a smaller subset of selected proteins, saving countless hours testing.
